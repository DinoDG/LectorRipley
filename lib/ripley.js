...
const CART_ID   = process.env.RIPLEY_CART_ID   || ""
...
const URL_CLEAR_CART = `${BASE_URL}/cart/clear`
...
// =========================================================
// TOKEN EN MEMORIA DEL SERVIDOR (equivalente a global TOKEN)
// =========================================================
...
async function limpiarCarrito(headers) {
  const res = await fetch(URL_CLEAR_CART, {
    method:  "PUT",
    headers,
  ...
  if (res.status === 401 || res.status === 403) {
    await obtenerToken()
    const h2  = await headersAuth()
    const re2 = await fetch(URL_CLEAR_CART, { method: "PUT", headers: h2, body: JSON.stringify({ cart_id: CART_ID }) })
    if (!re2.ok) throw new Error("Error limpiando carrito tras renovar token")
    return
  }
  if (!res.ok) throw new Error(`Error limpiando carrito: ${res.status}`)
}
...
export async function consultarPrecio(skuRaw) {
  ...
  if (sku.length === 12) {
    const digito = calcularDigitoEan13(sku)
    sku = sku + digito
  ...
  try {
    ...
    if (res.status === 401 || res.status === 403) {
      await obtenerToken()
      const h2 = await headersAuth()
      res = await fetch(URL_PRODUCTO, { method: "POST", headers: h2, body: JSON.stringify(payload) })
    }
