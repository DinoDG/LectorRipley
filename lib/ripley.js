...
export async function consultarPrecio(skuRaw, storeId = STORE_ID) {
  let sku = skuRaw.trim()
  ...
  try {
    ...
    return {
      ok: true,
      producto: {
        sku:                 p.ean13 || p.sku || sku,
        descripcion:         p.description,
        precio:              p.price_discount ?? p.price,
        precioTarjetaRipley: p.price_tr_discount ?? null,
        promo:               p.code_promotion_ripley || p.code_promotion || null,
        depto:               p.department,
        linea:               p.linecode,
